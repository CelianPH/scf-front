...
import { PawPrint, Lightbulb, AlertTriangle, Check, ArrowUp } from "lucide-react";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
import Reveal from "@/components/layout/Reveal";
import { CtaButton } from "@/components/ui/CtaButton";
import ConseilsFaq from "@/components/conseils/ConseilsFaq";
import { getConseilsPage } from "@/lib/strapi";
import { getIcon } from "@/lib/icons";

export async function generateMetadata(): Promise<Metadata> {
  const { data } = await getConseilsPage();
  const seo = data.seo;
  return {
    title:
      seo?.metaTitle ?? "Conseils : bien vivre avec son chat | Sans Croquettes Fixes",
    description:
      seo?.metaDescription ??
      "Le guide complet pour adopter et prendre soin d'un chat.",
  };
}

export default async function ConseilsPage() {
  const { data: page } = await getConseilsPage();
  const { hero, sections, kit, faq } = page;
              ...
              {sections.map((section) => {
                ...
                return (
                  <section
                    ...
                    aria-labelledby={`section-${section.id}-titre`}
          ...
          >
            <div className="mx-auto max-w-5xl px-5 py-14 md:px-8 md:py-20">
              ...
              <ul className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {kit.map((item, i) => (
                  <Reveal as="li" key={item.id} delay={Math.min(i, 8) * 50}>
        ...
        {faq.length > 0 ? (
          ...
          >
            <div className="mx-auto max-w-7xl px-5 py-14 md:px-8 md:py-20">
              ...
              <ConseilsFaq items={faq} />
            </div>
          </section>
        ) : null}

        {/* CTA final */}
        {page.ctaTitre || page.ctaPrimary || page.ctaSecondary ? (
            ...
            <div className="relative overflow-hidden">
              ...
              <Reveal className="relative mx-auto max-w-7xl px-5 py-14 text-center md:px-8 md:py-20">
                ...
                <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                  <CtaButton cta={page.ctaPrimary} />
                  <CtaButton cta={page.ctaSecondary} />
                </div>
              </Reveal>
            </div>
          </section>
